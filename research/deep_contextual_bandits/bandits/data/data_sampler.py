...
def sample_stats_questions_data(file_name, context_dim, num_actions, num_contexts,
                       shuffle_rows=False, shuffle_cols=False):
  """Samples bandit game from (expert, question) dense subset of Cross Validated Stack Exchange dataset.

  Args:
    file_name: Route of file containing the modified Stack dataset.
    context_dim: Context dimension (i.e. vector with some ratings from a user).
    num_actions: Number of actions (number of questions ratings to predict).
    num_contexts: Number of contexts to sample.
    shuffle_rows: If True, rows from original dataset are shuffled.
    shuffle_cols: Whether or not context/action questions are randomly shuffled.
